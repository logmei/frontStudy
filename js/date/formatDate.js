...
exports.getDayInWeek = getDayInWeek
var dateFormators = {
  __proto__: null,
...
function parse(value, format) {
  if (format) {
    ...
    var match = new RegExp(
      format.replace(/([-.*+?^${}()|[\]\/\\])/g, '$1').replace(/([yMdHms])\1*/g, function (all, w) {
        groups.push(w)
        return '\\s*(\\d+)?\\s*'
      })
    ).exec(value)
    ...
    return new Date(
      _obj.y || new Date().getFullYear(),
      _obj.M ? _obj.M - 1 : new Date().getMonth(),
      _obj.d || 1,
      _obj.H || 0,
      _obj.m || 0,
      _obj.s || 0
  ...
  return +obj
    ...
    : new Date(
      String(value)
        .replace(/(\d{4})\D*(\d\d?)\D*(\d\d?).*(\d\d?)\D*(\d\d?)\D*(\d\d?)/, '$1/$2/$3 $4:$5:$6')
        .replace(/(\d{4})\D*(\d\d?)\D*(\d\d?)/, '$1/$2/$3')
    )
}
/**
 * 创建当前日期对象的副本
 ...
 */
function dayLeft(date, year, month, day) {
  var tmp = new Date(date.getFullYear(), date.getMonth(), date.getDate())
  var offset = new Date(year, month - 1, day) - tmp
  if (offset) {
...
function getDayInWeek(date) {
  const arr = '日一二三四五六'
  return arr[date ? new Date(date).getDay() : new Date().getDay()]
}
